...
if __name__=="__main__":
    ...
    y, z, loss, neg_marginal_likelihood, KL_divergence, px_elem, kl_div= vae.autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob)
	...
	z_VALS_ORIG, px_val, kl_val= session.run([z, px_elem, kl_div], feed_dict={x_hat: train[0], x:train[0], keep_prob : 1})
	save_scattered_images(z_VALS_ORIG, train[1], path=exp_name+"/PMLR_map.png")
        ...
        #Px_val = 6*Px_val+Px_val_
        kl_val = kl_val.reshape((LEN,)) 
        px_val = px_val.reshape((LEN,))
        Px_val = px_val-30*kl_val
        print '**** Min(Px)={},  Max(Px)={} ****'.format(np.min(Px_val), np.max(Px_val))
        indices = np.argsort(Px_val)[::-1]
        from collections import Counter
        ipdb.set_trace()
        top = train[1][indices[:100]]
