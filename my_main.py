...
def get_clean(num=200, one_hot=False, reverse=False, labels=range(5), n=5):
...
def parse_args():
    ...
    parser.add_argument('--db', type=str, default='mnist',
                        help='mnist,cifar')
    ...
    parser.add_argument('--class_num', type=int, default=10, help='The number of epochs to run')

    parser.add_argument('--batch_size', type=int, default=128, help='Batch size')

    parser.add_argument('--load', type=bool, default=False,
                        help='Boolean for loading model')
...
def main(args):
    ...
    train_total_data, train_labels = get_clean(num=1000, labels=range(CLASS_NUM), reverse=True)
    indices = range(train_total_data.shape[0])
    import random
    random.shuffle(indices)
    train_total_data = train_total_data[indices]
    train_labels = train_labels[indices]
    train_size = train_total_data.shape[0]
    #train_total_data, train_size, _, _, test_data, test_labels = mnist_data.prepare_MNIST_data()
    test_data, test_labels= get_clean(num=1001, labels=range(CLASS_NUM), reverse=False, one_hot=True, n=CLASS_NUM)
    indices = range(test_data.shape[0])
    random.shuffle(indices)
    ...
    # network architecture
    y, z, loss, neg_marginal_likelihood, KL_divergence, elbo_elem, reconstr_loss  = vae.autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob)
    ...
    model_path = "models/model_class_"+str(CLASS_NUM)+"_dim_"+str(dim_z)+".ckpt"
    if DB == 'cifar':
    ...
    with tf.Session() as sess:
	    ...
	    if PMLR and dim_z == 2:
	        ...
	        PMLR.save_images(y_PMLR_img, name="/PMLR.jpg")

	        # plot distribution of labeled images
	        z_PMLR = sess.run(z, feed_dict={x_hat: x_PMLR, keep_prob : 1})
	        PMLR.save_scattered_image(z_PMLR,id_PMLR, name="/PMLR_map.jpg")
