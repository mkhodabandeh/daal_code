...
def save_scattered_images(z, id, path='scattered_image.jpg', z_range=None, title=None):
    N = 10
    if z_range is None:
       z_min = np.min(z, 0)
       z_max = np.max(z, 0)
...
if __name__=="__main__":
    ...
    choices = [0.01 * 2 ** i for i in range(20)]
    BETAS = [choices[int(BETA)]]
    #if ',' in BETA:
...
########### Create a classifier #####################
	...
	X,Y,Px, z_VALS= copy_shuffle(train[0], train[1], Px_val, z_VALS_ORIG)
	   ...
	   for ll in range(CLASS_NUM):
	      ...
	      if per_class_num == 1:
           ...
           exclude = random.sample(range(10), 5)
           init_pool = []
           ...
           print set(Y[sorted_temp_px][:2000])
	...
	daal_size2acc[rounded_acc].append(train_l[0].shape[0])
	...
	np.save(exp_dir+'ent.npy', entropies)
        ...
        for BETAA in BETAS:
	       ...
	       #    	labels=Y)
               ...
               train_l = (X[labeled_], to_one_hot(Y[labeled_], CLASS_NUM))
               classifier.set_weights(copy.deepcopy(weights))
               save_scattered_images(z_VALS[candidates], predictions[candidates], path=exp_dir+'/candidates_latent_{1}_{0}_before.png'.format(BETA, step), z_range=z_range, title='Before iteration '+str(step))
               ...
               save_scattered_images(z_VALS, predictions, path=exp_dir+'/all_latent_{1}_{0}_after.png'.format(BETA, step),z_range=z_range, title='After iteration '+str(step))
               save_scattered_images(z_VALS[candidates], predictions[candidates], path=exp_dir+'/candidates_latent_{1}_{0}_after.png'.format(BETA, step),z_range=z_range, title='After iteration '+str(step))
